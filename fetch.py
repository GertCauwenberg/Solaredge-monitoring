...
def read_optimizer(opt):
    ...
    t = opt.lastReport
    data['timestamp'] = "{0}-{1}-{2} {3}:{4}:{5}".format(t.year, t.month, t.day, t.hour, t.minute, t.second)
...
def save_data(dbh, inv, optimizers, now):
    ...
    cursor = dbh.cursor()
    try:
        cursor.execute(sql1, (now, inv['power'], inv['u_ac'], inv['u_dc'], inv['i_dc'], inv['e_day'], inv['e_total' ], inv['temp']))
    except mdb.MySQLError as e:
