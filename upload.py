...
def pvoutput_update(system, output):
    ...
    response = requests.post('https://pvoutput.org/service/r2/addbatchstatus.jsp', headers=headers, data=data)
...
for system in systems:
    ...
    if len(data) > 0:
        csv = create_csv(data)
        print("Uploading {0} results to pvupload for system {1}".format(len(data), system[0]))
